---
title: "US Macroeconomic Dashboard"
format: 
  dashboard
---
...
# Data Source: [FRED St. Louis](https://fred.stlouisfed.org/)
library(httr)
library(fredr)
library(dplyr)
library(ggplot2)
...
Delinquency_Rate <- Delinquency_Rate[, c(1, 2, (ncol(Delinquency_Rate) - 7):ncol(Delinquency_Rate))]
...
series_ids_payroll <- c("ADPWNUSNERSA", "ADPWINDMANNERSA", "ADPWINDCONNERSA",
                  "ADPWINDINFONERSA","ADPWINDPROBUSNERSA","ADPWINDLSHPNERSA",
                  "ADPWINDFINNERSA","ADPWINDEDHLTNERSA","ADPWINDNRMINNERSA","ADPWINDTTUNERSA")
Payroll_Private_Chg <- fetch_fred_data(series_ids = series_ids_payroll,
...
CPI_table_data <- CPI_table_data[, c(1, 2, (ncol(CPI_table_data) - 12):ncol(CPI_table_data))]
...
init_cont_claims <- init_cont_claims[, c(1, (ncol(init_cont_claims) - 36):ncol(init_cont_claims))]
...
lf_status_flows <- lf_status_flows[, c(1, (ncol(lf_status_flows) - 5):ncol(lf_status_flows))]
...
fiscal_ids_2 <- c("WRESBAL","H41RESPPALDKXAWNWW","WTREGEN")
fiscal_2 <- fetch_fred_data(series_ids = fiscal_ids_2, freq = "w")
'
treasury_statement_ids <- c("MTSO133FMS","MTSR133FMS","MTSDS133FMS","FYFSGDA188S",
                            "GFDEGDQ188S","","","")
treasury_statement'
...
payroll_graf <- plot_ly(Payroll_Private_Abs, type = 'bar') %>%
  add_trace(x = ~date, y = ~ADPWNUSNERSA, name = 'Total Private', type = 'scatter', mode = 'lines') %>%
  add_bars(x = ~date, y = ~ADPWINDMANNERSA, name = 'Manufacturing', visible = FALSE) %>%
  add_bars(x = ~date, y = ~ADPWINDCONNERSA, name = 'Construction', visible = FALSE) %>%
  add_bars(x = ~date, y = ~ADPWINDINFONERSA, name = 'Information', visible = FALSE) %>%
  add_bars(x = ~date, y = ~ADPWINDPROBUSNERSA, name = 'Services', visible = FALSE) %>%
  add_bars(x = ~date, y = ~ADPWINDLSHPNERSA, name = 'Health', visible = FALSE) %>%
  add_bars(x = ~date, y = ~ADPWINDFINNERSA, name = 'Financial', visible = FALSE) %>%
  add_bars(x = ~date, y = ~ADPWINDNRMINNERSA, name = 'Mining', visible = FALSE) %>%
  add_bars(x = ~date, y = ~ADPWINDTTUNERSA, name = 'Trade', visible = FALSE) %>%
  add_bars(x = ~date, y = ~ADPWINDLSHPNERSA, name = 'Leisure and Hospitality', visible = FALSE) %>% 
  layout(title = 'Payroll',
          ...
          buttons = list(
          list(
              label = "Breakdown", method = "update",
              args = list(
                list(visible = c(TRUE,FALSE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE)),
                list(title = "Private Payroll Breakdown"))),
          list(
              label = "Total", method = "update",
              args = list(
                list(visible = c(TRUE,TRUE,FALSE,FALSE,FALSE,FALSE,FALSE,FALSE,FALSE,FALSE,FALSE)),
                list(title = "Payroll")))))))
...
```{r, include=TRUE, echo = FALSE, warning = FALSE}
#| title: Change
payroll_graf_chg <- plot_ly(Payroll_Private_Chg) %>%
  add_trace(x = ~date, y = ~ADPWNUSNERSA, name = 'Total Private', type = 'scatter', mode = 'lines') %>%
  add_trace(x = PAYROLL$date, y = PAYROLL$value, name = 'Total', type = 'scatter', mode = 'lines') %>%
  add_bars(x = ~date, y = ~ADPWINDMANNERSA, name = 'Manufacturing', visible = FALSE) %>%
  add_bars(x = ~date, y = ~ADPWINDCONNERSA, name = 'Construction', visible = FALSE) %>%
  add_bars(x = ~date, y = ~ADPWINDINFONERSA, name = 'Information', visible = FALSE) %>%
  add_bars(x = ~date, y = ~ADPWINDPROBUSNERSA, name = 'Services', visible = FALSE) %>%
  ...
  layout(title = 'Payroll',
          xaxis = list(title = ''),
          yaxis = list(title = 'Persons', range = c(-2e5, 1e6)),
          hovermode = 'compare',
          barmode = 'stack',
          updatemenus = list(list(
          active = -1,
          ...
          list(
              label = "Breakdown", method = "update",
              args = list(
                list(visible = c(FALSE,FALSE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE)),
                list(title = "Private Payroll Breakdown"))),
          list(
              label = "Total", method = "update",
              args = list(
          ...
          list(
              ...
              args = list(
                list(visible = c(TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,
                                 ...
                                 FALSE,FALSE,FALSE,FALSE,FALSE)),
                list(title = "Private Payroll, Persons by Firm Size",
          ...
          list(
              ...
              args = list(
                list(visible = c(TRUE,FALSE,FALSE,FALSE,FALSE,FALSE,
                                 ...
                                 FALSE,FALSE,FALSE,FALSE,FALSE)),
                list(title = "Payroll Month Over Month, Persons by Firm Size",
          ...
          list(
              ...
              args = list(
                list(visible = c(TRUE,FALSE,FALSE,FALSE,FALSE,FALSE,
                                 FALSE,FALSE,FALSE,FALSE,FALSE,
                                 TRUE,TRUE,TRUE,TRUE,TRUE)),
                list(title = "Payroll Year over Year, Persons by Firm Size",
...
QUITS_RATE_graf <- plot_ly(Jolts, type = 'scatter', mode = 'lines') %>%
  ...
  layout(title = 'Quits Rate x Wage Growth Tracker',
         xaxis = list(title = ''),
         yaxis = list(title = ''),
...
### Measures on Labor Subutilization
...
cpi_sticky_graf <- plot_ly(cpi_sticky_pc1, type = 'scatter', mode = 'lines') %>%
  ...
  ##add_trace(x = ~date, y = ~CRESTKCPIXSLTRM159SFRBATL, name = 'Sticky Core Less Shelter') %>%
  #add_trace(x = ~date, y = ~STICKCPIXSHLTRM159SFRBATL, name = 'Sticky Less Shelter') %>%
  ...
  #add_trace(x = ~date, y = cpi_sticky_3maa$CRESTKCPIXSLTRM679SFRBATL, name = 'Sticky Core Less Shelter 3M', visible = FALSE) %>%
  #add_trace(x = ~date, y = cpi_sticky_3maa$STICKCPIXSHLTRM679SFRBATL, name = 'Sticky Less Shelter 3M', visible = FALSE) %>%
